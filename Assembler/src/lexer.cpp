...
TokenType Lexer::deduce_token_type(std::string& token_string)
{
    ...
    while (current_index != token_string.size())
    {
        if (!isdigit(token_string[current_index])
            && !(is_hex && Lexer::check_hex(token_string[current_index])))
        {
            flag = 0;
            break;
        }
    ...
    if (directives_map.find(token_string) != directives_map.end())
        return "DIRECTIVE";
...
Token Lexer::get_next_multichar_token()
{
    ...
    while (token_pointer < file_content.size()
        && (isalnum(file_content[token_pointer]) || file_content[token_pointer] == '_'
               || file_content[token_pointer] == '.'))
    {
