...
void SymbolTable::insert_symbol(
    std::string name, int value, int visibility, std::string section, int import_export)
{
...
void Assembler::fp_skip_handler()
{
    ...
    if (line_iterator->operands.front().type != IMMED
        || line_iterator->operands.front().literal < 0)
    {
...
void Assembler::fp_custom_section_handler()
{
    ...
    if (line_iterator->operands.size() == 2)
    {
        ...
        {
            if (rwx[i] == 'R' || rwx[i] == 'r')
                current_rwx = current_rwx | R_MASK;
            if (rwx[i] == 'W' || rwx[i] == 'w')
                current_rwx = current_rwx | W_MASK;
            if (rwx[i] == 'X' || rwx[i] == 'x')
                current_rwx = current_rwx | X_MASK;
        }
    }
    else
        current_rwx = 7;
...
void Assembler::fp_equ_handler()
{
    ...
    if (temp_node == 0)
    {
        symtab.insert_symbol(line_iterator->operands.front().symbol,
            line_iterator->operands.back().literal, LOCAL, "ABS", NONE);
    }
    else
    {
        if (temp_node->section == "UNDEF")
...
void Assembler::fp_instruction_handler()
{
    ...
    for (auto& op : line_iterator->operands)
    {
        ...
        else if (op.type == IMMED_SYM_VALUE || op.type == PCREL || op.type == REGINDDISP_SYM_VALUE
            || op.type == MEMDIR || op.type == ABS)
        {
            LC = LC + 3;
        }
        else if (op.type == REGDIR)
        {
            LC = LC + 1;
        }
        else if (op.type == REGINDDISP_IMMED)
        {
            if (op.literal == 0)
                LC = LC + 1;
            else if (op.is_byte)
...
void Assembler::first_pass()
{
    while (line_iterator != line_list.end())
    {
        ...
        else if (line_iterator->is_instruction)
        {
        ...
        for (auto& it : line_iterator->operands)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (temp_ptr->section != "ABS")
                    {
                        reltab.insert(current_section, custom_sections[current_section].size() - 4,
                            REL_16, it.symbol);
                    }
                }
            }
...
void Assembler::sp_byte_handler()
{
    if (current_rwx & W_MASK)
    {
        for (auto& it : line_iterator->operands)
        {
            ...
            else if (it.is_byte || (temp_ptr && temp_ptr->section == "ABS"))
            {
                if (current_section == ".data")
                {
...
bool Assembler::check_operands()
{
    ...
    {
        if (line_iterator->operands.front().type == IMMED
            || line_iterator->operands.front().type == IMMED_SYM_VALUE)
            return false;
        if (line_iterator->operands.front().is_byte && line_iterator->operands.back().is_word)
            return false;
        return true;
    }
    else
...
std::string Assembler::one_operand_instruction_handler(
    bool first_operand, bool first_operand_byte, bool first_operand_word, bool front)
{
    Operand op;
    if (front)
        op = line_iterator->operands.front();
    else
    ...
    if (line_iterator->operands_byte)
    {
        ...
        switch (op.type)
        {
            ...
            case IMMED_SYM_VALUE:
                Error::wrong_literal_width(line_iterator->src_line);
            case REGDIR:
                if (!op.reg.low && !op.reg.high || op.symbol == "psw")
                    Error::wrong_literal_width(line_iterator->src_line);
            ...
            case REGINDDISP_IMMED:
                ...
                else
                {
                    ...
                    return byte_to_hex(second_byte) + offset_bytes.substr(offset_bytes.size() - 2)
                        + offset_bytes.substr(0, 2);
                }
            case REGINDDISP_SYM_VALUE:
                ...
                reltab.insert(current_section, shtab.find_sh_node(current_section)->size + 1,
                    REL_16, op.symbol);
                return byte_to_hex(second_byte) + offset_bytes.substr(offset_bytes.size() - 2)
                    + offset_bytes.substr(0, 2);
            case PCREL:
                ...
                if (symtab.find_symbol(op.symbol)->section == current_section)
                {
                    ...
                    else
                    {
                        offset_bytes = word_to_hex(third_byte);
                        return byte_to_hex(second_byte)
                            + offset_bytes.substr(offset_bytes.size() - 2)
                            + offset_bytes.substr(0, 2);
                    }
                }
                else
    ...
    else if (line_iterator->operands_word)
    {
        int second_byte = 0;
        int third_byte = 0;
        int fourth_byte = 0;
        ...
        switch (op.type)
        {
            ...
            case IMMED_SYM_VALUE:
                ...
                reltab.insert(current_section, shtab.find_sh_node(current_section)->size + 1,
                    REL_16, op.symbol);
                return byte_to_hex(second_byte) + temp.substr(offset_bytes.size() - 2)
                    + temp.substr(0, 2);
            case REGDIR:
                if (op.reg.low || op.reg.high)
                    Error::wrong_literal_width(line_iterator->src_line);
                second_byte = (second_byte | REGDIR_MASK) << 5;
                second_byte = second_byte | (op.reg.number << 1);
                return byte_to_hex(second_byte);
        ...
        if (check_operands())
        {
            ...
            switch (op.type)
            {
                case IMMED:
                    ...
                    if (op.is_byte)
                    {
                        if (first_operand || first_operand_byte)
                            return byte_to_hex(second_byte) + byte_to_hex(op.literal);
                        else if (first_operand_word)
                        {
                            temp = word_to_hex(op.literal);
                            return byte_to_hex(second_byte) + temp.substr(temp.size() - 2)
                                + temp.substr(0, 2);
                        }
                    }
                    temp = word_to_hex(op.literal);
                    if (first_operand || first_operand_word)
                        return byte_to_hex(second_byte) + temp.substr(temp.size() - 2)
                            + temp.substr(0, 2);
                    else if (first_operand_byte)
                ...
                case REGDIR:
                    ...
                    second_byte = (second_byte | REGDIR_MASK) << 5;
                    second_byte = second_byte | (op.reg.number << 1);
                    if (first_operand_byte && !first_operand && !op.reg.low && !op.reg.high)
                        Error::wrong_operands_size(line_iterator->src_line);
                    return byte_to_hex(second_byte);
                case REGINDDISP_IMMED:
                    if (op.literal == 0)
                    {
                ...
                case PCREL:
                    ...
                    {
                        reltab.insert(current_section,
                            shtab.find_sh_node(current_section)->size + 1, REL_PC_16, op.symbol);
                        return byte_to_hex(second_byte) + word_to_hex(0);
                    }
                case ABS:
                    second_byte = (second_byte | MEMDIR_MASK) << 5;
                    reltab.insert(current_section, shtab.find_sh_node(current_section)->size + 1,
                        REL_16, op.symbol);
                    return byte_to_hex(second_byte)
                        + word_to_hex(symtab.find_symbol(op.symbol)->value);
                case MEMDIR:
                    second_byte = (second_byte | MEMDIR_MASK) << 5;
                    offset_bytes = word_to_hex(op.literal);
                    return byte_to_hex(second_byte) + offset_bytes.substr(offset_bytes.size() - 2)
    ...
    if (current_rwx & X_MASK)
    {
        if (line_iterator->operands.size() == instructions_op_numbers[line_iterator->instruction])
        {
            ...
            if (line_iterator->operands_word)
                first_byte = first_byte | WORD_SIZED_OPERANDS_MASK;
            ...
            switch (line_iterator->operands.size())
            {
                case 1:
                    ...
                    break;
                case 2:
                    two_operands_instruction_handler();
                    break;
            }
        }
        else
            Error::wrong_operands_numbers(line_iterator->instruction,
                instructions_op_numbers[line_iterator->instruction], line_iterator->src_line);
    }
    else
...
void Assembler::second_pass()
{
    ...
    while (line_iterator != line_list.end())
    {
        ...
        if (line_iterator->directive == ".end")
            break;
        line_iterator++;
    }
}
...
void RelocationTable::insert(
    std::string section, unsigned offset, RelocationType type, std::string symbol)
{
...
RelocationTableNode* RelocationTable::find(std::string section, unsigned offset)
{
    for (auto& ptr : reltab)
    {
        if (ptr->section == section && ptr->offset == offset)
            return ptr;
    }
    return 0;
}
