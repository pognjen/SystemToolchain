...
public:
    ...
    friend std::ostream& operator<<(std::ostream& os, const SymbolTableNode& node) // for testing
    {
        ...
        if (node.visibility == LOCAL)
            os << "LOCAL | " << node.section << " | ";
        else
            os << "GLOBAL | " << node.section << " | ";
        if (node.import_export == IMPORTED)
            os << "IMPORT";
        else if (node.import_export == EXPORTED)
            os << "EXPORT";
        else
            os << "NONE";
        return os;
...
public:
    ...
    friend std::ostream& operator<<(
        std::ostream& os, const SectionHeaderTableNode& n) // for testing
    {
